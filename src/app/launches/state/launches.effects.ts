...
export class LaunchesEffects {
  ...
  @Effect()
  getLaunches$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(state => {
      ...
      return this.launchesService.getLaunches(launchesState.params).pipe(
        map(queryResults => {
  ...
  @Effect()
  routeToLaunches$: Observable<{}> = this.actions$.pipe(
    ...
    map(action => {
      ...
      this.store.dispatch(
        new launches.SetOffset(parseInt(routeQueryParams.offset, 10))
      );
